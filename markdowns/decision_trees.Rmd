...
Cluster countries by most important social feature (happiness) for classification
```{r}
set.seed(42)
...
```{r}
# making two trees 
ct_sample_music <- rpartXse(cluster ~ ., train_music, se=0.5)
...
# contingency tables
cm_sample_music <- table(ps_sample_music,test_music$cluster)
...
Plot the tree
```{r}
prp(ct_sample_music, type = 1, extra = 103, roundint = FALSE)
