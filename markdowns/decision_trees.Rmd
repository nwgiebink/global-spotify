...
With an error rate of ~.41, random forest performed *slightly* worse than our single decision tree, which yielded an error rate of ~0.38. Surprisingly, variable importance changed: track.popularity is still most important, but speechiness is relatively unimportant in the random forest model; instead, the second most important variable here is whether music is explicit.

```{r, cache=TRUE}
# making a tree
set.seed(42)
ct_mus_age <- rpartXse(cluster ~ ., mus_train, control = list(maxdepth = 3))

# prediction using the trees

pred_mus_age <- predict(ct_mus_age, mus_test, type = "class")

# have a look at the variable.importance
ct_mus_age$variable.importance

# contingency tables
cm_mus_age <- table(pred_mus_age, mus_test$cluster)

# error rate
error_mus_age <- (1-sum(diag(cm_mus_age))/sum(cm_mus_age))
cat("DT on age clusters error rate: ",error_mus_age)

tree_mus_age <- prp(ct_mus_age, type = 1, extra = 103, roundint = FALSE)
```
