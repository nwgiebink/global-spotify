...
pop <- spot %>% select(track.popularity)
...
spot2_disc <- mutate_all(spot2, funs(disc))
track.popularity <- mutate(pop, track.popularity = 
                             if_else(track.popularity > median(pop$track.popularity), 
                                                   'high','low'))
spot_disc <- cbind(spot2_disc, track.popularity)
spot_disc$track.popularity <- factor(track.popularity$track.popularity)
...
rules <- apriori(spot_trans, parameter = list(support = 0.05, 
                                        confidence = 0.7,
                                        target = 'rules')) 
summary(rules) # returns ~4500 rules

# make less conservative rules set for exploring specific variables 
# Reason: Valence rules (rules_val) results in zero rules with thresholds:
# support = 0.05 and confidence = 0.7.

rules2 <- apriori(spot_trans, parameter = list(support = 0.025, 
                                        confidence = 0.6,
                                        target = 'rules')) 
summary(rules2) # returns 34538 rules
...
rules_val <- subset(rules2, subset = lhs %in% social & rhs %pin% 'valence=')

```

Associations are overwhelmed by rules with track.popularity=high on the rhs. Are each of the lhs sets meaningfully associated with high track.popularity, or is this just a byproduct from sampling tracks from Top 50 in each country (in which case we expect most tracks to have **high** popularity except in the case of tracks which are only popular in a few countries but are internationally obscure.

Question 1:
Are other levels of track.popularity frequent enough to exist in our set of rules? Associations with track.popularity=high would be more interesting if we could tease apart which social variables are **not** associated with high popularity.

```{r}
rules_pop <- subset(rules2, subset=(lhs %in% social &
                        !(lhs %in% music) & 
                        rhs %pin% 'track.popularity='))
...
# nodes and lines plot
plot(rules_sub, method="graph", engine="htmlwidget", max=30)
plot(rules_val, method = 'graph', engine='htmlwidget', max = 30)
